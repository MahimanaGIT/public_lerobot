...
def make_act_multiframe_pre_post_processors(
    ...
    """Creates the pre- and post-processing pipelines for the ACT Multi-Frame policy.

    The pre-processing pipeline handles normalization, batching, and device placement for the model inputs,
    including support for multi-frame temporal inputs.

    The post-processing pipeline handles unnormalization and moves the model outputs back to the CPU.

    Args:
        config (ACTMultiFrameConfig): The ACT Multi-Frame policy configuration object.
    ...
    Returns:
        ...
        A tuple containing the pre-processor pipeline and the post-processor pipeline.
    """
