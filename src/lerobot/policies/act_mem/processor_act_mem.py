...
def make_act_mem_pre_post_processors(
    config: ACTMemConfig,
    ...
    """Creates the pre- and post-processing pipelines for the ACT-MEM policy.

    The pre-processing pipeline handles normalization, batching, and device placement for the model inputs,
    including support for multi-frame temporal inputs and episodic memory.

    The post-processing pipeline handles unnormalization and moves the model outputs back to the CPU.

    Args:
        config (ACTMemConfig): The ACT-MEM policy configuration object.
